...
export default function BalancesPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchData = async (userId: string) => {
    ...
    try {
      ...
      const balRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL || (process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5001')}/api/groups/${groupId}/balances`, { headers });
      if (balRes.ok) {
  ...
  const handleExplainBalance = async (memberUserId: string, userObj: any) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || (process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5001')}/api/groups/${groupId}/explain-balance?userId=${memberUserId}`);
  ...
  const handleSettleDebt = async (fromUserId: string, toUserId: string, amount: number) => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || (process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5001')}/api/groups/${groupId}/settlements`, {
        method: 'POST',
        headers: { 
