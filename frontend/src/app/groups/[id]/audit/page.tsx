...
export default function AuditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchData = async (userId: string) => {
    ...
    try {
      // Fetch group name
      const groupRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL || (process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5001')}/api/groups/${groupId}`, {
      ...
      // Fetch audit logs
      const logsRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL || (process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5001')}/api/groups/${groupId}/audit-logs`, {
        headers: { 'x-user-id': userId },
      });
